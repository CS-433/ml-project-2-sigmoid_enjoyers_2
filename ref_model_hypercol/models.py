...
class UNetResNextHyperSE(nn.Module):
    ...
    def __init__(self, encoder_depth, num_classes, num_filters=32, dropout_2d=0.2, is_deconv=False):
        ...
        self.input_adjust = blocks.EncoderBlock(
            nn.Sequential(self.encoder.layer0.conv1, self.encoder.layer0.bn1, self.encoder.layer0.relu1, self.pool),
            num_filters * 2,
        )
        ...
        self.dec3 = blocks.DecoderBlockV4(bottom_channel_nr // 2 + num_filters * 8, num_filters * 8 * 2, num_filters * 8, is_deconv)
        self.dec2 = blocks.DecoderBlockV4(bottom_channel_nr // 4 + num_filters * 8, num_filters * 4 * 2, num_filters * 2, is_deconv)
        self.dec1 = blocks.DecoderBlockV4(bottom_channel_nr // 8 + num_filters * 2, num_filters * 2 * 2, num_filters * 2 * 2, is_deconv)

        # Adjusted to handle 28-channel input
        self.final = nn.Conv2d(28, self.num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Reduce concatenated output to the number of classes
        final_output = self.final(dsv0)

        # Upsample to match the size of the target labels
